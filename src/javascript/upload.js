...
const radianceOrIrradianceInput = document.getElementById(
...
const handleSubmit = async (event) => {
  ...
  const radianceOrIrradianceSelect = document.getElementById(
    "radiance-or-irradiance"
  );
  const areaUnitSelect = document.getElementById("area-units");
  const powerUnitSelect = document.getElementById("power-units");
  const radianceOrIrradiance =
    radianceOrIrradianceSelect.options[radianceOrIrradianceSelect.selectedIndex]
      .value;
  const areaScale = parseFloat(
    areaUnitSelect.options[areaUnitSelect.selectedIndex].value
  );
  const powerScale = parseFloat(
    powerUnitSelect.options[powerUnitSelect.selectedIndex].value
  );
...
radianceOrIrradianceInput.addEventListener(
  "change",
  (event) => {
