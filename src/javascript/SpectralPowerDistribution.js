export default class SpectralPowerDistribution {
  ...
  get interval () {
    ...
    for (let i = 0, {wavelengths} = this; i < wavelengths.length - 1; i += 2) {
  ...
  get measurements () {
    ...
    return [...Array(sampleCount).keys()].map((index) => this.rows.map(([wavelength, ...measurements]) => [wavelength, measurements[index]]));
  }
}
