...
const addDataSourcesToChart = (
  ...
  const toggleDataSource = (event) => {
    ...
    if (event.target.value === "raw") {
      yAxisLabel = `Spectral ${radianceOrIrradiance} [${radianceOrIrradianceSIUnit(
        radianceOrIrradiance
      )}]`;
      data = rows;
        ...
        yAxes: [
          {
            scaleLabel: {
              display: true,
              labelString: `Spectral ${radianceOrIrradiance} [${radianceOrIrradianceSIUnit(
                radianceOrIrradiance
              )}]`,
            },
          },
