...
import { uvToCorrelatedColourTemperatureOhnoRounded } from "./cctCalculations";
...
export const planckianRelativeSPD = (lambda, T) => {
  ...
  const c2 = 1.4388e-2;
  const sr = math.round(
    c1 / math.pow(lambda, 5) / (Math.exp(c2 / (T * lambda)) - 1),
    15
  );
  return sr;
};
...
export const convertToCAMUCS = (XrYrZr, tcsVal) => {
  ...
  const k = math.round(1 / (1 + 5 * LA), 15);
  const FL = math.round(
    0.2 * k ** 4 * 5 * LA + 0.1 * (1 - k ** 4) ** 2 * (5 * LA) ** (1 / 3),
    15
  );

  const n = math.round(Yb / XrYrZr.Source.Y, 15); // 0.2 from Eq. 11(c)
  const nbb = math.round(0.725 * math.pow(1 / n, 0.2), 15);
  const ncb = math.round(0.725 * math.pow(1 / n, 0.2), 15);
  const z = 1.48 + math.pow(n, 1 / 2);
  ...
  const [Rc, Gc, Bc] = [
    math.round((D * (Yw / Rw) + 1 - D) * R, 15),
    math.round((D * (Yw / Gw) + 1 - D) * G, 15),
    math.round((D * (Yw / Bw) + 1 - D) * B, 15),
  ];
  const [Rcw, Gcw, Bcw] = [
    math.round((D * (Yw / Rw) + 1 - D) * Rw, 15),
    math.round((D * (Yw / Gw) + 1 - D) * Gw, 15),
    math.round((D * (Yw / Bw) + 1 - D) * Bw, 15),
  ];
  ...
  // Eq. 10
  const a = math.round(Rprimea - (12 * Gprimea) / 11 + Bprimea / 11, 15);
  const b = math.round((1 / 9) * (Rprimea + Gprimea - 2 * Bprimea), 15);

  // Eq. 11
  const A = math.round(
    (2 * Rprimea + Gprimea + (1 / 20) * Bprimea - 0.305) * nbb,
    15
  );
  const Aw = math.round(
    (2 * Rprimeaw + Gprimeaw + (1 / 20) * Bprimeaw - 0.305) * nbb,
    15
  );

  const J = math.round(100 * (A / Aw) ** (c * z), 15);
  ...
  if (a === 0 && b === 0) {
    ...
    h = math.round((360 / (2 * Math.PI)) * Math.atan2(b, a), 15);
  } else {
    h = math.round(360 + (360 / (2 * Math.PI)) * Math.atan2(b, a), 15);
  }

  const e = math.round(
    (12500 / 13) * ncb * Nc * (Math.cos((h * Math.PI) / 180 + 2) + 3.8),
    15
  ); // Eq. 12(f)
  const t = math.round(
    (e * (a ** 2 + b ** 2) ** 0.5) / (Rprimea + Gprimea + (21 / 20) * Bprimea),
    15
  ); // Eq. 12(e)

  const bigC = math.round(
    math.pow(t, 0.9) *
      (J / 100) ** 0.5 *
      math.pow(1.64 - math.pow(0.29, n), 0.73),
    15
  ); // Eq. 12(d)
  const M = math.round(bigC * math.pow(FL, 0.25), 15); // Colourfulness, Eq. 12(c)

  const Mprime = math.round((1 / 0.0228) * Math.log(1 + 0.0228 * M), 15);

  return {
    Jprime: math.round(((1 + 100 * 0.007) * J) / (1 + 0.007 * J), 15),
    aprime: math.round(Mprime * Math.cos((Math.PI / 180) * h), 15),
    bprime: math.round(Mprime * Math.sin((Math.PI / 180) * h), 15),
  };
};
...
export const toColourFidelityIndex = (dE) => {
  ...
  if (typeof dE === "number") {
    ...
    Rf = math.round(10 * Math.log(Math.E ** ((100 - cf * dE) / 10) + 1), 15);
  } else {
    // otherwise, it should be the dictionary/json object
    Object.keys(dE).forEach((key) => {
      // 10 * LN(EXP((100 - cf * G30) / 10) + 1)
      const tmpRfi = math.round(
        10 * Math.log(Math.E ** ((100 - cf * dE[key]) / 10) + 1),
        15
      );
      Rf[key] = tmpRfi;
    });
 ...
 * @returns
 */
export const calculateColourFidelityIndexRounded = (spectra) => {
  intermediateValuesRf = {
    u: "N/A",
    v: "N/A",
    uPrime: "N/A",
    vPrime: "N/A",
  ...
  const { CCT: T, Duv } = uvToCorrelatedColourTemperatureOhnoRounded(u, v);
  if (T <= 0) {
  ...
  Object.keys(Jabt).forEach((key) => {
    if (key !== "Source") {
      const t = Jabt[key];
      const r = Jabr[key];
      const tmpEi = math.round(
        math.pow(
          math.pow(r.Jprime - t.Jprime, 2) +
            math.pow(r.aprime - t.aprime, 2) +
            math.pow(r.bprime - t.bprime, 2),
          0.5
        ),
        15
      );
  ...
  const deltaEbar = math.round((1 / 99) * sum, 15); // off after 6th decimal place precision
